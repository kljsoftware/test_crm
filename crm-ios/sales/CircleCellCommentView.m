...
@implementation CircleCellCommentView
...
- (void)setCommentItemsArray:(NSArray *)commentItemsArray
{
    ...
    for (int i = 0; i < commentItemsArray.count; i++) {
        Comment *model = commentItemsArray[i];
...
- (NSMutableAttributedString *)generateAttributedStringWithCommentItemModel:(Comment *)model
{
    ...
    [attString setAttributes:@{NSLinkAttributeName : [NSString stringWithFormat:@"%ld",model.uid]} range:[text rangeOfString:model.uname]];
    if (model.bname.length) {
        if ([model.bname isEqualToString:model.uname]) {
            [attString setAttributes:@{NSLinkAttributeName : [NSString stringWithFormat:@"%ld",model.buid]} range:[text rangeOfString:[@" " stringByAppendingString:model.bname]]];
        }else{
