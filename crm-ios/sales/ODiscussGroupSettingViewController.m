//
//  ODiscussGroupSettingViewController.m
//  sales
//
...
@implementation ODiscussGroupSettingViewController
...
- (void)viewDidLoad {
    ...
    if (self.conversationType == ConversationType_DISCUSSION) {
        ...
        __weak OSettingBaseViewController *weakSelf = self;
                ...
                success:^(RCDiscussion *discussion) {
                    if (discussion) {
                        ...
                        for (NSString *targetId in discussion.memberIdList) {
                            NSString *fid = [targetId substringFromIndex:4];
                            Contact * contact = [_dbUtil selectUserById:[fid integerValue]];
                            if (contact != nil && contact.fid > 0) {
                                ...
                                NSString* urlStr = [NSString stringWithFormat:@"%@%@",BASE_URL,API_FIND_BYID];
                                
                                NSMutableURLRequest *request = [[AFHTTPRequestSerializer serializer] requestWithMethod:@"POST" URLString:urlStr parameters:@{@"id":fid}                                                                                    error:nil];
                                    ...
                                    if (error) {
                                        ...
                                        if (responseObject) {
                                            ...
                                            if ([dictionary[@"result"] intValue] == 1) {
                                                Contact *resp = [Contact mj_objectWithKeyValues:dictionary[@"data"]];
                                                resp.fid = resp.id;
                                                [users addObject:resp];
                                                
                                                [_members setObject:contact forKey:[NSString stringWithFormat:@"%ld",resp.fid]];
                                                if (users.count == discussion.memberIdList.count) {
                                                    [weakSelf addUsers:users];
                                                    [_userList addObjectsFromArray:users];
                                                }
...
//    点击最后一个+号,调出选择联系人UI
    if (indexPathOfSelectedItem.row == settingTableViewHeader.users.count) {
        OSelectFriendsViewController *contactSelectedVC= [[OSelectFriendsViewController alloc]init];
...
- (void)deleteTipButtonClicked:(NSIndexPath *)indexPath {
    ...
    [[RCIM sharedRCIM] removeMemberFromDiscussion:self.targetId
                                          ...
                                          success:^(RCDiscussion *discussion) {
                                              ...
                                              [self.members removeObjectForKey:[NSString stringWithFormat:@"%ld",user.fid]];
