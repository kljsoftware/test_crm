//
//  WorkDetailsTableViewController.m
//  sales
//
...
@interface WorkDetailsTableViewController ()
@property (nonatomic, strong) MBProgressHUD *hud;
@property (nonatomic,assign) NSInteger p;
@property (nonatomic,assign) NSInteger isRefresh;
@end

@implementation WorkDetailsTableViewController
...
- (void)viewDidLoad {
    ...
    WorkDetailsHeaderView *headView = [WorkDetailsHeaderView new];
    headView.model = _work;
...
- (void)getCommentList{
    ...
    NSString* urlStr = [NSString stringWithFormat:@"%@%@",BASE_URL,API_WORK_COMMENT_LIST];
    NSInteger workid = _work.id;
    _p = _p + 1;
    NSString *page = [NSString stringWithFormat:@"%ld",_p];
    NSMutableURLRequest *request = [[AFHTTPRequestSerializer serializer] requestWithMethod:@"POST" URLString:urlStr parameters:@{@"page":page,@"pagenum":@"10",@"workid":[NSString stringWithFormat:@"%ld",workid]}                                                                                   error:nil];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.didWorkCommentSelected) {
        self.didWorkCommentSelected(comment);
    }
