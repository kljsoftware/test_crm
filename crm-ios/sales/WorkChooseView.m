...
#define LAND_SPACE      (KSCREEN_WIDTH - 4*ITEM_WIDTH - 2*LAND_EDGE)/3
#define PORT_SPACE      20.0
#define ITEM_WIDTH      40.0
#define ITEM_HEIGHT     (ITEM_WIDTH + 18.0 + 4.0)
#define CANCEL_HEIGHT   49.0

@interface WorkChooseView () <UIGestureRecognizerDelegate>

@property (nonatomic, strong) UIView *bgView;
@property (nonatomic, strong) NSArray *items;
@property (nonatomic, assign) CGFloat bgHeight;

@property (nonatomic, copy) WorkChooseBlock block;

@end

@implementation WorkChooseView
...
- (instancetype)initWithItem:(NSArray *)items selectIndex:(WorkChooseBlock)block {
    ...
    if (self = [super init]) {
        ...
        NSInteger lineNum = items.count%4 == 0 ? items.count/4 : items.count/4 + 1;
